        ...
        public HttpResponseMessage CheckDataSource([FromUri] string[] parameters)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    masterSwitch = CheckSwitch("Master", false); //Check master switch, if it's off we'll avoid any other checks
                    if (masterSwitch)
                    {
                        ...
                        if(switchResult)
                        {
                            ...
                            if(!overrideReadinessQuery) //Check override switch. If it's flipped we won't do any further checks 
                            {
                                if (result.Rows[0].Field<string>(0) != "" && result.Rows[0].Field<string>(1) != "")
                                {
                                    ...
                                    try
                                    {
                                        DataTable queryResultTable = ExtractFactory.ConnectAndQuery(connection, query, 10);
                    ...
                    {
                        UniversalLogger.WriteToLog(procLog, "The " + parameters[0] + " database is not ready. Master switch is turned off.", category: UniversalLogger.LogCategory.WARNING);
                        sourceReady = false;
                        if(parameters[0] != "Master")
                        {
                            switchResult = CheckSwitch(parameters[0], false);
                        }
                        else
                        {
                            switchResult = masterSwitch;
                        }
